...
//	Padding and border style widths for mouse offsets
	...
	var permissionForm = new function () {
		...
		function userDropListEvent(e) {
			...
			//alert(data);
			if(data.length > 30){
				alert ("Google Image cannot be dragged, please download as local file!");
			}
			else{
...
//					new_node.className = '';
					//alert(currentlyDraggedNode.src);
//					this.appendChild(new_node);
					var img3 = new Image();
					img3.src = currentlyDraggedNode.src;
...
//	then add everything we want to intially exist on the canvas
	function init2() {
		canvas = document.getElementById('restrictedUsers');
		HEIGHT = canvas.height;
		WIDTH = canvas.width;
		ctx = canvas.getContext('2d');
		ctx.fillText("Drop an image onto the canvas", 100, 150);		
...
//		img = new Image();
//		img.src = "http://localhost:8080/RockTee/images/pink%20skull.png";
//		addRect(60, 70, 60, 65,img);
...
//	function saveimage(){
////	var cs = new CanvasSaver('http://localhost:8080/RockTee/scripts/saveme.php');
//	var cs = new CanvasSaver('http://greenethumb.com/canvas/lib/saveme.php');
//	alert("hh");
//	cs.generateButton('save an image!', canvas, 'myimage');
//	}
...
//	removes a node
	function myDblClick(e) {
		...
		for (var i = l-1; i >= 0; i--) {
			...
			if (imageData.data[3] > 0) {
				var mySel = boxes2[i];
				if (mx >= mySel.x && mx <= mySel.x + mySel.w &&
						my >= mySel.y && my <= mySel.y + mySel.h) {
//					alert(i);
