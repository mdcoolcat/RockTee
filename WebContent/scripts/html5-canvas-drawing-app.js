...
var clickX_real = new Array();
var clickY_real = new Array();
var clickColor_real = new Array();
var clickTool_real = new Array();
var clickSize_real = new Array();
var clickDrag_real = new Array();
var clickColor = new Array();
var clickTool = new Array();
var clickSize = new Array();
var clickDrag = new Array();
var paint = false;
var paint_real = false;
...
function prepareCanvas()
{
	...
	realcanvas = document.getElementById('restrictedUsers');
	...
	realcanvas.addEventListener("mousedown", function (evt) {
		if(document.getElementById('doodle_mode').checked){
			paint_real = true;
			addClick_real(evt.pageX - 260, evt.pageY - 400, false);
			redraw_real();
		}
	}, false);

	realcanvas.addEventListener("mousemove", function(evt){			//alert(paint_real);

		if(paint_real==true && document.getElementById('doodle_mode').checked){
			addClick_real(evt.pageX - 260, evt.pageY - 400, true);
			//alert(evt.pageX+ " "+evt.pageY +" " +this.offsetLeft+" "+ this.offsetTop);
			//redraw_real();
...
//			ctx.stroke();
			var i = 0;
			var radius;
			for(; i < clickX_real.length; i++)
			{		
				if(clickSize_real[i] == "small"){
					radius = 2;
				}else if(clickSize_real[i] == "normal"){
					radius = 5;
				}else if(clickSize_real[i] == "large"){
					radius = 10;
				}else if(clickSize_real[i] == "huge"){
					radius = 20;
				}else{
				...
				ctx.beginPath();
				if(clickDrag_real[i] && i){
					ctx.moveTo(clickX_real[i-1], clickY_real[i-1]);
					//alert("X: i-1 "+clickX_real[i-1] + "Y:i-1 "+clickY_real[i-1]);

				}else{
					ctx.moveTo(clickX_real[i], clickY_real[i]);
				}
				//alert("X:i "+clickX_real[i] + "Y:i "+clickY_real[i]);

				ctx.lineTo(clickX_real[i], clickY_real[i]);
				ctx.closePath();
				if(clickTool_real[i] == "eraser"){
					...
					ctx.strokeStyle = clickColor_real[i];
				}
				//ctx.lineJoin = "round";
				ctx.lineWidth = radius;
//alert(i);
				//alert("i:"+i+" radisu:"+radius+" style:"+ctx.strokeStyle+" X: "+clickX_real[i]+" Y: "+clickY_real[i]);
				ctx.stroke();
			}
		}
	}, false);

	realcanvas.addEventListener("mouseup", function(evt){
		if(document.getElementById('doodle_mode').checked){
			paint_real = false;
			redraw_real();
		}
	}, false);
	realcanvas.addEventListener("mouseleave", function(evt){
		if(document.getElementById('doodle_mode').checked){
			paint_real = false;
		}
	}, false);
...
function addClick_real(x, y, dragging)
{
	clickX_real.push(x);
	clickY_real.push(y);
	clickTool_real.push(curTool);
	clickColor_real.push(curColor);
	clickSize_real.push(curSize);
	clickDrag_real.push(dragging);
}
